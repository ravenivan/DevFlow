        ...
        return (
          <SheetClose asChild key={item.route}>
            <Link
              href={item.route}
              className={`${
                isActive ? 'primary-gradient text-light-900 rounded-lg' : 'text-dark-300_light-900'
...
const MobileNav = () => {
  return (
    <Sheet>
      ...
      <SheetContent side="left" className="background-light-900_dark-200 border-none p-6">
        <SheetTitle className='sr-only'>Navigation Menu</SheetTitle>

        <Link href="/" className="flex items-center gap-1">
          <Image src="/assets/images/site-logo.svg" width={23} height={23} alt="DevFlow" />

          <p className="h2-bold text-dark-100_light-900 font-spaceGrotesk">
